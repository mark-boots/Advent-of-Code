...
const beams = [];
for(let y = 0; y < GRID_H; y++) beams.push( [y, -1, 1], [y, GRID_W, 3] );
for(let x = 0; x < GRID_W; x++) beams.push( [-1, x, 2], [GRID_H, x, 0] );
...
function getEnergizedTiles(beamStart) {
    const beam = [ beamStart ];
    const seen = new Set();
